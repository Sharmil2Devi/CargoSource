...
function CallData() {
    ...
    $.ajax({
        type: 'post',
        url: $('#hf_GetConsigneeData').val(),
        datatype: 'json',
        success: function (response) {
            FetchData(response.aConsigneeList);
            $('#LoadingImage').hide();
        },
            ...
            columns: [
...

                {
                    title: "<center>Actions</center>", "bSortable": false, "render": function (data, type, full, meta) {
                        return '' +
                            '<i class="fa fa-trash" aria- title="Delete" onclick=DeleteConsigneeDetails(' + isNullCheck(data) + ');></i >' +
                            '<i class="fa fa-pencil" aria- title="Withdrawn" data-col="Name" onclick="GoToConsigneeDetails(' + isNullCheck(data) + ');"></i>' +
                            '';
                    }
                }
            ],
...
function DeleteConsigneeDetails(aitem) {

    bootbox.confirm("Are you sure to delete the Consignee?",
        function (result) {
            if (result) {
                var data = { aConsigneeID: aitem };
                $.ajax({
                    type: 'post',
                    url: $('#hf_ConsigneeInfoDelete').val(),
                    data: data,
                    datatype: 'json',
                    success: function (response) {
...
function GoToConsigneeDetails(item) {
    ClearForm();
    aID = item;
    if (aID != 0) {

        var data = { zConsigneeID: aID }
        $.ajax({
            type: 'post',
            url: $('#hf_GetConsigneeDetails').val(),
            data: data,
            datatype: 'json',
            success: function (response) {
...
function UpdateConsigneeData() {
    ...
    if (zResult) {
        ...
        $.ajax({
            type: 'post',
            url: $('#hf_UpdateConsigneeDetails').val(),
            data: data,
            datatype: 'json',
            success: function (response) {
                if (response.toString().indexOf('Error') != -1) {
